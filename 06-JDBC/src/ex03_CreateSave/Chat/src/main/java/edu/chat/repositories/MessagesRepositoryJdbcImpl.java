...
import edu.chat.exception.NotSavedSubEntityException;
...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
  ...
  private void saveUser(Message message) {
    ...
    //    String queryUpdate =
    //        "insert into chat.User(user_id, user_login, user_password) values (?, ?, ?);";
    //    try (Connection connection = hikariDataSource.getConnection();
    //         PreparedStatement statementUpdate = connection.prepareStatement(queryUpdate)) {
    //      statementUpdate.setLong(1, message.getAuthor().getId());
    //      statementUpdate.setString(2, message.getAuthor().getLogin());
    //      statementUpdate.setString(3, message.getAuthor().getPassword());
    //      statementUpdate.executeUpdate();
    //    } catch (Exception e) {
    //      throw new RuntimeException(e);
    //    }
  ...
  private void saveChatroom(Message message) {
    ...
    //    String queryUpdate =
    //        "insert into chat.Chatroom(chatroom_id, chatroom_name, chatroom_owner) values (?, ?,
    //        ?);";
    //    try (Connection connection = hikariDataSource.getConnection();
    //         PreparedStatement statementUpdate = connection.prepareStatement(queryUpdate)) {
    //      statementUpdate.setLong(1, message.getRoom().getId());
    //      statementUpdate.setString(2, message.getRoom().getName());
    //      statementUpdate.setLong(3, message.getRoom().getOwner().getId());
    //      statementUpdate.executeUpdate();
    //    } catch (Exception e) {
    //      throw new RuntimeException(e);
    //    }
